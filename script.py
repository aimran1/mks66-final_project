...
def first_pass( commands ):
  ...
  if vary_found and not frames_found:
    print('Parsing failed: vary defined but frames undefined')
    return
  if frames_found and not name_found:
    name = 'my_animation'
    print ('Basename used: \'my_animation\'')
...
def second_pass( commands, num_frames ):
  ...
  for command in commands:
    if command['op'] == 'vary':
      ...
      first = float(command['args'][2])
      second = float(command['args'][3])
      knob_name = command['knob']
      add_me = (second-first)/(end_frame-start_frame)
      for frame in range(start_frame,end_frame):
        first += add_me
        frames[frame][knob_name] = first
  return frames
